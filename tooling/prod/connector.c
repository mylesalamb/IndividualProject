...
int send_quic_http_request(char *host, char *sni, int locport, int ecn)
{
        ...
        const char *key_file = NULL, *val, *port_str;
        ...
        // h3cli.h3cli_local_sas = &saddr;


        host_to_sockaddr(host, 443, &addr, &socklen);

        // we can do this in consruct sock to host
        h3cli.h3cli_hostname = sni;
        h3cli.h3cli_path = "/";

        /* Resolve hostname */
        h3cli.h3cli_sock_fd = construct_sock_to_host(&addr, &socklen, locport, SOCK_DGRAM);
     ...
     */
        ...
        settings.es_ecn = ecn;
        settings.es_versions = LSQVER_ID29;
...
//         struct lsquic_engine_t *engine;
//         struct lsquic_engine_api eapi;
//         struct lsquic_engine_settings settings;
//         struct quic_cb_data ctx;
//         ctx.sni = request;
//         ctx.method = "GET";
...
//         settings.es_ecn      = ecn;
//         settings.es_versions = LSQVER_ID29; // This seems to be best option in terms of support + ecn

//         if(lsquic_engine_check_settings(&settings, LSENG_HTTP, errbuff, sizeof(errbuff))){
//                 fprintf(stderr, "quic_request:settings\n");
//         }

//         engine = lsquic_engine_new(LSENG_HTTP, &eapi);

//         lsquic_engine_connect(engine, N_LSQVER, );
